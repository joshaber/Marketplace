...
#define MAATTACHEDWINDOW_DEFAULT_BACKGROUND_COLOR [NSColor colorWithCalibratedWhite:0.1f alpha:0.75f]
...
- (void)_updateGeometry
{
    ...
    CGFloat halfWidth = contentRect.size.width / 2.0f;
    CGFloat halfHeight = contentRect.size.height / 2.0f;
    switch (_side) {
        case MAPositionTopLeft:
...
- (MAWindowPosition)_bestSideForAutomaticPosition
{
    ...
    CGFloat halfWidth = viewSize.width / 2.0f;
    CGFloat halfHeight = viewSize.height / 2.0f;
...
- (CGFloat)_arrowInset
{
    ...
    return (cornerInset + (arrowBaseWidth / 2.0f)) * MAATTACHEDWINDOW_SCALE_FACTOR;
}
...
- (NSColor *)_backgroundColorPatternImage
{
    ...
    if (borderWidth > 0) {
        ...
        [bgPath setLineWidth:(borderWidth * 2.0f) * MAATTACHEDWINDOW_SCALE_FACTOR];
        [borderColor set];
        [bgPath stroke];
    }
...
- (NSBezierPath *)_backgroundPath
{
    ...
    CGFloat halfArrowWidth = scaledArrowWidth / 2.0f;
    NSRect contentArea = NSInsetRect(_viewFrame,
                                     -viewMargin * scaleFactor,
                                     -viewMargin * scaleFactor);
    CGFloat minX = NSMinX(contentArea) * scaleFactor;
    CGFloat midX = NSMidX(contentArea) * scaleFactor;
    CGFloat maxX = NSMaxX(contentArea) * scaleFactor;
    CGFloat minY = NSMinY(contentArea) * scaleFactor;
    CGFloat midY = NSMidY(contentArea) * scaleFactor;
    CGFloat maxY = NSMaxY(contentArea) * scaleFactor;
...
- (void)setCornerRadius:(CGFloat)value {
    CGFloat maxRadius = ((MIN(_viewFrame.size.width, _viewFrame.size.height) + 
                        (viewMargin * 2.0f)) - arrowBaseWidth) / 2.0f;
    if (value <= maxRadius) {
        cornerRadius = value;
    } else {
        cornerRadius = maxRadius;
    }
    cornerRadius = MAX(cornerRadius, 0.0f);
